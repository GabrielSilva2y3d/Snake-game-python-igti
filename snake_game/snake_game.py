...
import math                     #realizar operações matematicas
...
class cube(object):
    ...
    def draw(self, surface, eyes=False):
        ...
        i = self.pos[0]                                                     #posiçãoo das linhas (grid)
...
class snake(object):
    body = []                           #lista que contém o corpo de cubos da snake
...
#método utilizado para realizar os movimentos da snake
    ...
    def addCube(self):
        ...
        #identifica em qual direção está se movendo para adicionar o cubo á cauda
        if dx == 1 and dy == 0:                                      #movimentando para a direita
            self.body.append(cube((tail.pos[0]-1,tail.pos[1])))      #adiciona o cubo á esquerda
...
def main():
    ...
    win = pygame.display.set_mode((width, width))               #define as dimensçes da tela
    ...
    while flag:
        ...
        s.move()                                                #captura os movimentos (teclas) e realiza a atualização da posição da snake
        ...
        for x in range(len(s.body)):
            if s.body[x].pos in list(map(lambda z:z.pos,s.body[x+1:])):         #caso a cabeça encontre uma parte do corpo
                print("\'Pontua��o: \'", len(s.body))                           #realiza o print da pontuação final 
